...
class BaseSegmentationModel(BaseModel):
    ...
    def get_probs_and_seg(self, image):
        ...
        print(f"mask_cls_result Min: {mask_cls_result.min()}, Max: {mask_cls_result.max()}")
        print(f"mask_pred_result Min: {mask_pred_result.min()}, Max: {mask_pred_result.max()}")

        mask_cls_result = F.softmax(mask_cls_result, dim=1).cpu().numpy()
        mask_pred_result = mask_pred_result.sigmoid().cpu().numpy()

        # Print the min and max values of soft_mask
        print(f"mask_cls_result (after) Min: {mask_cls_result.min()}, Max: {mask_cls_result.max()}")
        print(f"mask_pred_result (after) Min: {mask_pred_result.min()}, Max: {mask_pred_result.max()}")
